...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        msg_id = esp_mqtt_client_subscribe(client, "/stu_207/lamp2", 0);

        msg_id = esp_mqtt_client_unsubscribe(client, "/topic/qos1");
        ESP_LOGI(TAG, "sent unsubscribe successful, msg_id=%d", msg_id);
                // เพิ่มบรรทัดต่อไปนี้ โดยตั้งขื่อเป็น stu-<เลข 3 ตัวท้ายของรหัสนักศึกษา>
        msg_id = esp_mqtt_client_subscribe(client, "/stu_207/lamp1", 0);
        ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
        break;
    ...
    case MQTT_EVENT_DATA:
            ...
            if (strncmp(event->topic, "/stu_207/lamp2", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
            {
                ESP_LOGI(TAG, "event->topic = /stu_207/lamp2");
                if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
                {
...
![image](https://github.com/TikPoramat2545/MQTT_Lab_II/assets/134470274/dd4fff6d-8dcf-4355-8d43-3aeec777ad77)
