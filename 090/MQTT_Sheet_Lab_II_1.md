...
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/e99e8a9e-9808-4316-88d4-763590ef1519)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/61d48e77-310f-4adc-9361-3877bc3b1ecf)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/ef8169eb-f8b2-4b5e-aa44-a9960aa82b00)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/638bf4f4-68d1-4b65-b47b-4c858e10cb80)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/4553a8c8-af75-48ab-987d-1f862214f8e6)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/fedea361-5d80-4127-ad02-793d12465d0c)
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/4073a6f9-679d-4998-bb6e-733e8bdfcf68)
...
#define LED_GPIO_PIN 2 // Change this to the GPIO pin to which your LED is connected
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
    	...
    	ESP_LOGI(TAG, "sent unsubscribe successful, msg_id=%d", msg_id);
    	        ...
    	        // เพิ่มบรรทัดต่อไปนี้ โดยตั้งขื่อเป็น stu-<เลข 3 ตัวท้ายของรหัสนักศึกษา>
    	msg_id = esp_mqtt_client_subscribe(client, "/stu_090/lamp1", 0);
    	ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
    ...
    case MQTT_EVENT_DATA:
        ...
        if (strncmp(event->topic, "/stu_090/lamp1", event->topic_len) == 0) {
            ESP_LOGI(TAG, "event->topic = /stu_090/lamp1");
            if (strncmp(event->data, "1", event->data_len) == 0) {
                ESP_LOGI(TAG, "Turn on LED");
                gpio_set_level(LED_GPIO_PIN, 1); // Turn on LED (assuming active HIGH)
                led_state = true;
            }
            if (strncmp(event->data, "0", event->data_len) == 0) {
                ESP_LOGI(TAG, "Turn off LED");
                gpio_set_level(LED_GPIO_PIN, 0); // Turn off LED (assuming active HIGH)
                led_state = false;
            }
        }
        break;
...
void app_main(void)
{
    ...
    gpio_set_level(LED_GPIO_PIN, 0); // Initialize LED as OFF (assuming active HIGH)
...
![image](https://github.com/Nanapon2002/MQTT_Lab_II/assets/115066356/c7bcb1fb-fde4-433f-9386-229224d86d47)
